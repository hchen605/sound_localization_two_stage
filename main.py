...
loss_type = 'MAE' #mean absolute error
################# param #################
batch_size = 32
Max_epochs = 50
lr = 1e-3
...
mel_bins = 96
frames_per_1s = fs // hopsize #//operator aka floor division
...
#https://www.itread01.com/content/1520672540.html hdf5
...
def train(args, data_generator, model, optimizer, logging):
    ...
    iterator = tqdm(enumerate(data_generator.generate_train()),#Gen batch for train
        total=Max_epochs*epoch_size, unit='batch')

    for batch_idx, (batch_x, batch_y_dict) in iterator: #batch_x: (batch_size, mic_channels, seq_len, freq_bins) batch_y_dict: {'events': target_events, 'elevation': target_elevation, 'azimuth': target_azimuth, 'distance': target_distance}
        ...
        # Backward
        optimizer.zero_grad() #https://meetonfriday.com/posts/18392404/
        seld_loss.backward()
        optimizer.step()
...
def inference(args, data_generator, logging):
    ...
    if args.task_type == 'two_staged_eval':
        ...
        #check path
        print('if model path:', model_path)
        ...
        model_path = os.path.join(models_dir, 'doa_only',
                                ...
                                'iter_{}.pth'.format(args.iteration))
        #check path
        print('else model path:', model_path)
...
def inference_all_fold(args):
    ...
    gt_meta_dir = '/home/hchen605/dcase2019/dataset/metadata_dev/'
...
if __name__ == '__main__':
    ...
    if args.task_type == 'sed_only' or args.task_type == 'seld':
        ...
        args.model = Model_DOA #Model_DOA = 'pretrained_CRNN10'
    ...
    logging.info('\n===> Building model')
    #load CRNN model from models, args.model = Model_DOA #Model_DOA = 'pretrained_CRNN10'
    model = models.__dict__[args.model](class_num, args.model_pool_type, 
        args.model_pool_size, args.model_interp_ratio, pretrained_path)
    ...
    data_generator = DataGenerator(
        ...
        hdf5_dir=hdf5_dir,
        logging=logging
    )#get train/val, test feature, labels, index,
