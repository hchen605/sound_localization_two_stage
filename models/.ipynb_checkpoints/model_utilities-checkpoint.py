...
def interpolate(x, ratio):
    ...
    #upsampled = x[:, :, None, :].detach().cpu().numpy().repeat(ratio, axis=2)
    #print('upsampled = ')
    #print(upsampled[:2,:3,:2,:4])
    #upsampled = np.repeat(x[:, :, None, :], ratio, axis=2)
    upsampled = upsampled.reshape(batch_size, time_steps * ratio, classes_num)
    #print('upsampled reshape = ')
    #print(upsampled[:2,:3,:4])
    #numpy to torch
    #upsampled = torch.from_numpy(upsampled).cuda().requires_grad_()
    #print('upsampled torch = ')
    #print(upsampled[:2,:3,:4])
...
# https://discuss.pytorch.org/t/what-kind-of-filters-are-used-with-conv2d/34406/5
